 ...
 * @retval Número de caracteres lidos
 */
...
int main(void)
{
  ...
  int index = 0; // Índice de escrita na Flash (controla a posição do "array")
  ...
  // receber dataset via UART
  while (1)
  {
      printTX("[RX] aguardando...\r\n");
      read_line(rx, sizeof(rx));
      printTX("[RX] recebido: '%s'\r\n", rx);

      // Verifica se é o comando de parada "FIM"
      if (strncmp(rx, "FIM", 3) == 0)
      {
          printTX("[RX] FIM recebido.\r\n");
          break;
      }

      // Tenta ler a string no formato "float,float"
      // sscanf retorna o número de itens que conseguiu ler com sucesso.
      if (sscanf(rx, "%f,%f", &x, &y) == 2) // espera 2 itens
      {
          printTX("[FLASH] gravando: x=%.3f y=%.3f index=%d\r\n", x, y, index);

          // Grava os dois floats na Flash.
          flash_dataset_write_float(index++, x);
          flash_dataset_write_float(index++, y);
      }
      else
      {
          printTX("[ERRO] linha invalida\r\n");
      }
  }


  // Treinar
  printTX(">> Iniciando treino...\r\n");
  gradient_run();
  printTX(">> Treino finalizado.\r\n");


  printTX(">> Enviando resultado...\r\n");
