...
NUMERO_EXECUCOES = 20
TAMANHO_BATCH = 1000  # 1000 amostras (Flash)
NOME_ARQUIVO_SAIDA = 'resultados_monolitico.txt'
# =================================================
...
def realizar_ciclo(ser, dados_para_enviar, n_ciclo):
    ...
    print(f"[PYTHON] Enviando batch de {len(dados_para_enviar)} pontos (Monolítico/Flash).")

    # 1. Aguarda STM32 (Apagamento da Flash)
    ...
    # 2. Envia Dados
    print(f"[ENVIO] Transmitindo dados...")
    for x, y in dados_para_enviar:
        ser.write(f"{x},{y}\n".encode())
        time.sleep(0.002) 
        # Espera o ACK do microcontrolador para cada ponto
        while True:
            try:
    ...
    # 3. Finaliza envio e aguarda processamento
    ser.write(b"FIM\n")
    ...
    while True:
        ...
        if "RESULTADO:" in linha:
            ...
            # Regex para extrair a=..., b=..., mse=..., TEMPO=..., EPOCAS=...
...
def salvar_relatorio_txt(dados_coletados):
    ...
    with open(NOME_ARQUIVO_SAIDA, 'w', encoding='utf-8') as f:
        f.write("=================================================================================\n")
        f.write(f" RELATÓRIO: TREINAMENTO MONOLÍTICO (FLASH - 1000 PONTOS)\n")
        f.write("=================================================================================\n\n")
        f.write(f"Data/Hora: {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"Dataset: {ARQUIVO_CSV}\n")
        f.write(f"Amostras: {TAMANHO_BATCH} (Ordem Fixa)\n")
...
def main():
    ...
    # Seleciona os primeiros 1000 pontos (Ordem Fixa)
    batch_fixo = pontos_todos[:TAMANHO_BATCH]
    print(f"[CONFIG] Batch de {len(batch_fixo)} pontos FIXADO (Sem Shuffle).")

    resultados = []
    try:
        for i in range(NUMERO_EXECUCOES):
            # Envia exatamente o mesmo batch, na mesma ordem
