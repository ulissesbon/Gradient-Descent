...
import re
import math
# import random  <-- Não é mais necessário para embaralhar
...
NUMERO_EXECUCOES = 20
TAMANHO_BATCH = 1000  
NOME_ARQUIVO_SAIDA = 'resultados_flash_unrolling.txt'
# =================================================
...
def realizar_ciclo(ser, dados_para_enviar, n_ciclo):
    ...
    print(f"[PYTHON] Enviando batch de {len(dados_para_enviar)} pontos (Ordem Fixa).")

    # 1. Aguarda STM32 (Ele demora um pouco para apagar a Flash)
    print("[AGUARDANDO] Esperando 'READY' do STM32 (Flash Erase)...")
    while True:
        try:
    ...
    # 2. Envia Dados
    print(f"[ENVIO] Transmitindo dados para Flash...")
...
def salvar_relatorio_txt(dados_coletados):
    ...
    with open(NOME_ARQUIVO_SAIDA, 'w', encoding='utf-8') as f:
        f.write("=================================================================================\n")
        f.write(f" RELATÓRIO: TREINAMENTO FLASH + UNROLLING (SEM SHUFFLE)\n")
        f.write("=================================================================================\n\n")
        f.write(f"Data/Hora: {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"Dataset Base: {ARQUIVO_CSV}\n")
        f.write(f"Amostras: {TAMANHO_BATCH} (Ordem Sequencial Fixa)\n")
...
def main():
    ...
    # --- SELEÇÃO FIXA DE DADOS ---
    # Seleciona os primeiros 1000 pontos.
    ...
    try:
        for i in range(NUMERO_EXECUCOES):
            # Envia exatamente o mesmo batch, na mesma ordem, todas as vezes
