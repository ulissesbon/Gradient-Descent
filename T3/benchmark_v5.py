...
# ================= CONFIGURAÇÕES =================
PORTA_SERIAL = '/dev/ttyACM0'  # Ajuste conforme seu sistema (COM3, /dev/ttyUSB0, etc)
BAUDRATE = 115200
ARQUIVO_CSV = 'data/original/dataset_original0.csv' 
NUMERO_EXECUCOES = 20
TAMANHO_BATCH = 550  # 550 Pontos conforme definido no dataset_flash.h
NOME_ARQUIVO_SAIDA = 'resultados_flash_modular_shuffle.txt'
# =================================================
...
def realizar_ciclo(ser, dados_batch, n_ciclo):
    print(f"\n--- EXECUÇÃO {n_ciclo + 1}/{NUMERO_EXECUCOES} ---")
    
    # --- EMBARALHAMENTO ---
    # Cria uma cópia da lista fixa e embaralha a ordem
    dados_enviar = dados_batch[:]
    random.shuffle(dados_enviar)
    
    print(f"[PYTHON] Enviando {len(dados_enviar)} pontos (Ordem Aleatória)...")

    # 1. Aguarda STM32 (Flash Erase pode demorar)
    ...
    while True:
        ...
        if linha:
            # print(f"[DEBUG] {linha}") # Descomente para ver logs da placa
            if "READY" in linha:
                break

    # 2. Envia Dados
    print(f"[ENVIO] Transmitindo para Flash...")
...
def salvar_relatorio_txt(dados_coletados):
    ...
    with open(NOME_ARQUIVO_SAIDA, 'w', encoding='utf-8') as f:
        f.write("=================================================================================\n")
        f.write(f" RELATÓRIO: FLASH MODULAR 550 PONTOS (SHUFFLED)\n")
        f.write("=================================================================================\n\n")
...
def main():
    ...
    if len(pontos_todos) < TAMANHO_BATCH:
        print(f"[ERRO] Dataset insuficiente para o batch de {TAMANHO_BATCH}.")
        return

    # --- SELEÇÃO FIXA DE VALORES ---
    # Selecionamos os 550 pontos UMA ÚNICA VEZ. 
    # Isso garante que todos os testes rodem com os MESMOS números, apenas em ordem diferente.
    ...
    try:
        for i in range(NUMERO_EXECUCOES):
            # Passamos o batch fixo; a função realizar_ciclo irá embaralhar a ordem
