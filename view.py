...
def update(frame):
    ...
    return linha, linha_erro

anim = FuncAnimation(fig, update, frames=len(a_hist), interval=100, blit=True)
plt.tight_layout()
plt.show()
...
plt.quiver(AA, BB, DA, DB, color='gray', alpha=0.7)
plt.plot(a_hist, b_hist, 'r.-', label='Caminho (a,b)')
plt.xlabel('a')
plt.ylabel('b')
plt.title('Campo de Gradiente e Caminho da Descida')
plt.legend()
plt.grid(True)
plt.show()
